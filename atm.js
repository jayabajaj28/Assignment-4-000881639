...
//Using a constant variable to store the API endpint for fetching the records
const API_ENDPOINT =
  ...
  if (AcctNo && Pin && choice) {
    ...
    switch (choice) {
      ...
      case 3:
        ...
        if (withdrawalAmount) {
          ...
          if (updatedBalance != "Error") {
            ...
            //show error message if user doesn't have sufficient balance
            document.getElementById("SubMsg").innerHTML =
      ...
      case 4:
        ...
        break;
      default:
        //In case the choice is incorrect, throw alert to ask user to enter appropriate details
        window.alert(
...
//fetches the records from the API and based on the number of records creates a table on HTML
function showDatabase() {
  //using fetch API, get the DB from the endpoint
  fetch(API_ENDPOINT)
    .then((response) => {
      if (response.ok) {
        return response.json();
      }
      //throw error in case there are no records
      throw new Error("Network response was not ok.");
    })
    //Using the API data create the customer and account instances
    .then((data) => {
      ...
      //Create table in HTML for all the records
      //header row
      let table =
